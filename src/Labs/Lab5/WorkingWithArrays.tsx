...
export default function WorkingWithArrays() {
  ...
  return (
    <div id="wd-working-with-arrays">
      ...
      <br /><br /><hr />
      <a href={`${API}/${todo.id}/completed/${todo.completed}`} className="checkbox float-end">
        Update Completed</a>
      <input defaultValue={todo.id} className="form-control w-25 float-start me-2"
        onChange={(e) => setTodo({ ...todo, id: e.target.value })}/>
      <input type="checkbox" defaultChecked={todo.completed} onChange={(e) => setTodo({ ...todo, completed: e.target.checked })}/>
      <br /><br /><hr />
    </div>
);}